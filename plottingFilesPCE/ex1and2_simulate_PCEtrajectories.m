...
ex_to_plot = 2; %choose 1 for ex1 and 2 for ex2

if ex_to_plot ==1
    sys.p = 4;
    [sys,fns] = initializeVDPEP(sys,fns); 
    xini = kron([1;1.5],[1;zeros(sys.p,1)]);
elseif ex_to_plot ==2
    sys.p = 3;
    [sys,fns] = initializeIannelli(sys,fns); 
    xini = kron([0.8;0.8],[1;zeros(sys.p,1)]);
else
    error('not a valid example number, please choose between 1 and 2')
end
...
hold on
if ex_to_plot ==1
    title('Ex1: PCE trajectories','FontSize',15)
else 
    title('Ex2: PCE trajectories','FontSize',15)
end
